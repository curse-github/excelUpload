import { SystemError, UserError } from "@microsoft/teamsfx-api";
/**
 * Azure token/credential is invalid (usually not happen because TTK will ask user to login)
 */
export declare class InvalidAzureCredentialError extends SystemError {
    constructor();
}
/**
 * Azure subscription not available in the login tenant
 * may caused by user's account it not right
 */
export declare class InvalidAzureSubscriptionError extends UserError {
    constructor(subscriptionId: string);
}
/**
 * Failed to select subscription in current account.
 */
export declare class SelectSubscriptionError extends UserError {
    constructor();
}
/**
 * Resource group name exists when trying to create it
 */
export declare class ResourceGroupConflictError extends UserError {
    constructor(resourceGroupName: string, subscriptionId: string);
}
/**
 * Resource group does not exist when try to use it
 */
export declare class ResourceGroupNotExistError extends UserError {
    constructor(resourceGroupName: string, subscriptionId: string);
}
/**
 * Create resource group error
 */
export declare class CreateResourceGroupError extends UserError {
    constructor(resourceGroupName: string, subscriptionId: string, message: string);
}
/**
 * Check resource group existence error
 */
export declare class CheckResourceGroupExistenceError extends UserError {
    constructor(resourceGroupName: string, subscriptionId: string, message: string);
}
/**
 * List resource groups error
 */
export declare class ListResourceGroupsError extends UserError {
    constructor(subscriptionId: string, message: string);
}
/**
 * Get resource group error
 */
export declare class GetResourceGroupError extends UserError {
    constructor(resourceGroupName: string, subscriptionId: string, message: string);
}
/**
 * List resource group locations error
 */
export declare class ListResourceGroupLocationsError extends UserError {
    constructor(subscriptionId: string, message: string);
}
//# sourceMappingURL=azure.d.ts.map