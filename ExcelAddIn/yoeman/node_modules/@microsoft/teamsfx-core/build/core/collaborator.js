...
exports.validateEnvQuestion = exports.getQuestionsForListCollaborator = exports.getQuestionsForGrantPermission = exports.grantPermission = exports.checkPermission = exports.listCollaborator = exports.CollaborationUtil = exports.CollaborationConstants = void 0;
...
class CollaborationUtil {
    ...
    static async getTeamsAppIdAndAadObjectId(inputs) {
        ...
        const aadAppManifestFilePath = inputs === null || inputs === void 0 ? void 0 : inputs[question_2.CoreQuestionNames.AadAppManifestFilePath];
    ...
    static async loadManifestId(manifestFilePath) {
        try {
            if (!manifestFilePath || !(await fs_extra_1.default.pathExists(manifestFilePath))) {
                return teamsfx_api_1.err(new common_1.FileNotFoundError(constants_1.SolutionSource, manifestFilePath));
            }
            const manifest = await fs_extra_1.default.readJson(manifestFilePath);
            if (!manifest || !manifest.id) {
                return teamsfx_api_1.err(new teamsfx_api_1.UserError(constants_1.SolutionSource, constants_1.SolutionError.InvalidManifestError, localizeUtils_1.getLocalizedString("error.collaboration.InvalidManifestError", manifestFilePath)));
            }
            const id = manifest.id;
            return teamsfx_api_1.ok(id);
        }
        catch (error) {
            return teamsfx_api_1.err(new teamsfx_api_1.UserError(constants_1.SolutionSource, constants_1.SolutionError.FailedToLoadManifestFile, localizeUtils_1.getLocalizedString("error.collaboration.FailedToLoadManifest", manifestFilePath)));
        }
    }
    static requireEnvQuestion(appId) {
        return !!appId.match(CollaborationConstants.placeholderRegex);
    }
    static async parseManifestId(appId, inputs) {
        var _a;
...
async function listCollaborator(ctx, inputs, envInfo, tokenProvider, telemetryProps) {
    ...
    const hasTeams = tools_1.isV3Enabled() ? appIds.teamsAppId != undefined : true;
    ...
    const appStudioRes = hasTeams
        ? await appStudio.listCollaborator(ctx, inputs, envInfo, tokenProvider.m365TokenProvider, tools_1.isV3Enabled() ? appIds.teamsAppId : undefined)
        : teamsfx_api_1.ok([]);
    ...
    if (inputs.platform === teamsfx_api_1.Platform.CLI || inputs.platform === teamsfx_api_1.Platform.VSCode) {
        ...
        if (hasTeams) {
            message.push(...getPrintEnvMessage(tools_1.isV3Enabled() ? inputs.env : envInfo.envName, localizeUtils_1.getLocalizedString("core.collaboration.StartingListAllTeamsAppOwners")), {
        ...
        if (hasAad) {
            message.push(...getPrintEnvMessage(tools_1.isV3Enabled() ? inputs.env : envInfo.envName, localizeUtils_1.getLocalizedString("core.collaboration.StartingListAllAadAppOwners")), {
        ...
        if (inputs.platform === teamsfx_api_1.Platform.CLI) {
            ...
            ctx.userInteraction.showMessage("info", localizeUtils_1.getLocalizedString("core.collaboration.ListCollaboratorsSuccess", constants_2.VSCodeExtensionCommand.showOutputChannel), false);
    ...
    if (telemetryProps) {
        ...
        telemetryProps[constants_1.SolutionTelemetryProperty.CollaboratorCount] = teamsOwnerCount.toString();
...
async function checkPermission(ctx, inputs, envInfo, tokenProvider, telemetryProps) {
    ...
    const appStudio = typedi_1.Container.get(constants_3.ComponentNames.AppManifest);
    const aadPlugin = typedi_1.Container.get(constants_3.ComponentNames.AadApp);
    const isTeamsActivated = tools_1.isV3Enabled() ? appIds.teamsAppId != undefined : true;
    const appStudioRes = isTeamsActivated
        ? await appStudio.checkPermission(ctx, inputs, envInfo, tokenProvider.m365TokenProvider, userInfo, tools_1.isV3Enabled() ? appIds.teamsAppId : undefined)
        : teamsfx_api_1.ok([]);
    if (appStudioRes.isErr()) {
        return teamsfx_api_1.err(appStudioRes.error);
    }
...
async function grantPermission(ctx, inputs, envInfo, tokenProvider, telemetryProps) {
    ...
    try {
        ...
        const aadPlugin = typedi_1.Container.get(constants_3.ComponentNames.AadApp);
        const appStudioRes = isTeamsActivated
            ? await appStudio.grantPermission(ctx, inputs, envInfo, tokenProvider.m365TokenProvider, userInfo, tools_1.isV3Enabled() ? appIds.teamsAppId : undefined)
            : teamsfx_api_1.ok([]);
        if (appStudioRes.isErr()) {
            return teamsfx_api_1.err(appStudioRes.error);
        }
...
async function getQuestionsForGrantPermission(inputs) {
    ...
    if (isDynamicQuestion) {
        ...
        root.addChild(new teamsfx_api_1.QTreeNode(question_1.getUserEmailQuestion(jsonObject.upn)));
        return teamsfx_api_1.ok(root);
    }
    return teamsfx_api_1.ok(undefined);
}
exports.getQuestionsForGrantPermission = getQuestionsForGrantPermission;
async function getQuestionsForListCollaborator(inputs) {
    const isDynamicQuestion = teamsfx_api_1.DynamicPlatforms.includes(inputs.platform);
    if (isDynamicQuestion) {
        const root = await getCollaborationQuestionNode(inputs);
        return teamsfx_api_1.ok(root);
...
exports.getQuestionsForListCollaborator = getQuestionsForListCollaborator;
...
async function getCollaborationQuestionNode(inputs) {
    const root = new teamsfx_api_1.QTreeNode(selectAppTypeQuestion());
    // Teams app manifest select node
    const teamsAppSelectNode = question_2.selectTeamsAppManifestQuestion(inputs);
    ...
    const aadAppSelectNode = question_2.selectAadAppManifestQuestion(inputs);
    aadAppSelectNode.condition = { contains: CollaborationConstants.AadAppQuestionId };
    root.addChild(aadAppSelectNode);
    // Env select node
    const envNode = await question_2.selectEnvNode(inputs);
    if (!envNode) {
        return root;
    }
...
async function validateEnvQuestion(input, inputs) {
    if ((inputs === null || inputs === void 0 ? void 0 : inputs.env) || (inputs === null || inputs === void 0 ? void 0 : inputs.targetEnvName)) {
        return "Env already selected";
    }
    const appType = inputs === null || inputs === void 0 ? void 0 : inputs[CollaborationConstants.AppType];
    ...
    const aadManifestPath = inputs === null || inputs === void 0 ? void 0 : inputs[question_2.CoreQuestionNames.AadAppManifestFilePath];
    const teamsManifestPath = inputs === null || inputs === void 0 ? void 0 : inputs[question_2.CoreQuestionNames.TeamsAppManifestFilePath];
...
exports.validateEnvQuestion = validateEnvQuestion;
