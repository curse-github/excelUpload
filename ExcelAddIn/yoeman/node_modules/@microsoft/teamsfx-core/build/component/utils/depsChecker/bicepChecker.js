...
async function ensureBicep(ctx, inputs) {
    ...
    try {
        ...
        if (!(await bicepChecker.isGlobalBicepInstalled())) {
            await displayLearnMore(localizeUtils_1.getLocalizedString("error.common.InstallSoftwareError", bicepChecker.getBicepDisplayBicepName()), common_1.bicepHelpLink, ctx.ui || ctx.userInteraction, ctx.telemetryReporter);
...
function outputErrorMessage(ctx, bicepChecker, inputs) {
    var _a;
    const message = localizeUtils_1.getLocalizedString("error.common.InstallSoftwareError", bicepChecker.getBicepDisplayBicepName());
