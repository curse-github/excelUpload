...
exports.developerPortalScaffoldUtils = exports.isFromDevPortal = exports.updateScope = exports.getTemplateId = exports.TabUrlType = exports.DeveloperPortalScaffoldUtils = exports.answerToReplaceMessageExtensionBotId = exports.answerToRepaceBotId = void 0;
...
async function updateManifest(ctx, appDefinition, inputs) {
    ...
    const colorFilePath = path_1.default.join(ctx.projectPath, appPackageFolderName, colorFileName);
    const outlineFilePath = path_1.default.join(ctx.projectPath, appPackageFolderName, outlineFileName);
    ...
    if (inputs[question_1.CoreQuestionNames.ReplaceContentUrl] &&
        ...
        updateTabUrl(inputs[question_1.CoreQuestionNames.ReplaceContentUrl], TabUrlType.ContentUrl, tabs, existingManifestTemplate.staticTabs);
    }
    if (inputs[question_1.CoreQuestionNames.ReplaceWebsiteUrl] &&
        inputs[question_1.CoreQuestionNames.ReplaceWebsiteUrl].length != 0) {
        needUpdateStaticTabUrls = true;
        updateTabUrl(inputs[question_1.CoreQuestionNames.ReplaceWebsiteUrl], TabUrlType.WebsiteUrl, tabs, existingManifestTemplate.staticTabs);
    }
    ...
    if (manifest.developer) {
        ...
        if (!manifest.developer.name) {
            manifest.developer.name = constants_1.DEFAULT_DEVELOPER.name;
        }
        if (!manifest.description.short) {
            manifest.description.short = constants_1.DEFAULT_DESCRIPTION.short;
        }
        if (!manifest.description.full) {
            manifest.description.full = constants_1.DEFAULT_DESCRIPTION.full;
        }
    }
...
function getTemplateId(teamsApp) {
    ...
    if (utils_1.needTabAndBotCode(teamsApp)) {
        return { projectType: "tab-bot-type", templateId: constants_2.TabNonSsoAndDefaultBotItem().id };
    }
    // tab only
    if (utils_1.needTabCode(teamsApp)) {
        return { projectType: "tab-type", templateId: constants_2.TabNonSsoItem().id };
    }
    // bot and message extension
    if (utils_1.isBotAndMessageExtension(teamsApp)) {
        return { projectType: "bot-me-type", templateId: constants_2.DefaultBotAndMessageExtensionItem().id };
    }
    // message extension
    if (utils_1.isMessageExtension(teamsApp)) {
        return { projectType: "me-type", templateId: constants_2.MessageExtensionNewUIItem().id };
    }
    // bot
    if (utils_1.isBot(teamsApp)) {
        return { projectType: "bot-type", templateId: constants_2.BotOptionItem().id };
    }
    return undefined;
}
