...
class AzureDeployImpl extends baseDeployImpl_1.BaseDeployImpl {
        ...
        for (let i = 0; i < deployConstant_1.DeployConstant.DEPLOY_CHECK_RETRY_TIMES; ++i) {
            try {
                ...
                if (axios_1.default.isAxiosError(e)) {
                    await logger.error(`Check deploy status failed with response status code: ${(_b = (_a = e.response) === null || _a === void 0 ? void 0 : _a.status) !== null && _b !== void 0 ? _b : "NA"}, message: ${JSON.stringify((_c = e.response) === null || _c === void 0 ? void 0 : _c.data)}`);
                    throw new deploy_1.CheckDeploymentStatusError(location, new Error(`status code: ${(_e = (_d = e.response) === null || _d === void 0 ? void 0 : _d.status) !== null && _e !== void 0 ? _e : "NA"}, message: ${JSON.stringify((_f = e.response) === null || _f === void 0 ? void 0 : _f.data)}`), this.helpLink);
                }
                throw new deploy_1.CheckDeploymentStatusError(location, e, this.helpLink);
            }
            if (res) {
                if ((res === null || res === void 0 ? void 0 : res.status) === commonConstant_1.HttpStatusCode.ACCEPTED) {
                    ...
                    if (((_g = res.data) === null || _g === void 0 ? void 0 : _g.status) === deployConstant_1.DeployStatus.Failed) {
                        await this.logger.warning(localizeUtils_1.getDefaultString("error.deploy.DeployRemoteStartError", location, JSON.stringify(res.data)));
                    }
                    return res.data;
                }
                else {
                    ...
                    throw new deploy_1.CheckDeploymentStatusError(location, new Error(`status code: ${(_h = res.status) !== null && _h !== void 0 ? _h : "NA"}`), this.helpLink);
                }
            }
        }
        throw new deploy_1.CheckDeploymentStatusTimeoutError(this.helpLink);
    }
    /**
    ...
    async createAzureDeployConfig(azureResource, azureCredential) {
        ...
        try {
            ...
            if (token) {
                await this.logger.info("Get AAD token successfully. Upload zip package through AAD Auth mode.");
                return {
                    headers: {
                ...
                await this.logger.info("Get AAD token failed. AAD Token is empty. Upload zip package through basic auth mode. Please check your Azure credential.");
            }
        }
        catch (e) {
            this.context.telemetryReporter.sendTelemetryErrorEvent("Get-Deploy-AAD-token-failed", {
            ...
            await this.logger.info(`Get AAD token failed with error: ${JSON.stringify(e)}. Upload zip package through basic auth mode.`);
        }
        // IF only enable AAD deploy, throw error
        if (process.env["TEAMSFX_AAD_DEPLOY_ONLY"] === "true") {
            throw new deploy_1.GetPublishingCredentialsError(azureResource.instanceId, azureResource.resourceGroupName, new Error("Get AAD token failed."), this.helpLink);
        }
        const managementClient = this.managementClient;
        const listResponse = await azureSdkErrorHandler_1.wrapAzureOperation(() => managementClient.webApps.beginListPublishingCredentialsAndWait(azureResource.resourceGroupName, azureResource.instanceId), (e) => new deploy_1.GetPublishingCredentialsError(azureResource.instanceId, azureResource.resourceGroupName, e, this.helpLink), (e) => new deploy_1.GetPublishingCredentialsError(azureResource.instanceId, azureResource.resourceGroupName, e, this.helpLink));
    ...
    async restartFunctionApp(azureResource) {
        ...
        try {
            ...
            await this.logger.warning(localizeUtils_1.getLocalizedString("driver.deploy.error.restartWebAppError"));
        }
    }
}
