...
class YamlCompletion {
    ...
    doComplete(document, position, isKubernetes = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            else if (node && yaml_1.isScalar(node) && node.value === null && currentWord === '-') {
                overwriteRange = node_1.Range.create(position, position);
                this.arrayPrefixIndentation = ' ';
            }
            else {
            ...
            const collector = {
                add: (completionItem) => {
                    const addSuggestionForParent = function (completionItem) {
                        ...
                        const schemaType = schemaUtils_1.getSchemaTypeName(schema);
            ...
            try {
                ...
                if (node) {
                    ...
                    if (yaml_1.isMap(node) && node.items.length && yaml_1.isPair(node.items[0])) {
                        const pairNode = node.items[0];
                        if (yaml_1.isScalar(pairNode.value) &&
                            yaml_1.isScalar(pairNode.key) &&
                    ...
                    if (yaml_1.isSeq(node) && node.items.length && yaml_1.isScalar(node.items[0]) && lineContent.includes('-')) {
                        const nullNode = node.items[0];
                        if (nullNode.value === null && // value is null
                        ...
                        for (const key in schemaProperties) {
                            if (Object.prototype.hasOwnProperty.call(schemaProperties, key)) {
                                ...
                                if (typeof propertySchema === 'object' && !propertySchema.deprecationMessage && !propertySchema['doNotSuggest']) {
                                    ...
                                    const isNodeNull = (yaml_1.isScalar(originalNode) && originalNode.value === null) ||
                                        (yaml_1.isMap(originalNode) && originalNode.items.length === 0);
    ...
    addEnumValueCompletions(schema, separatorAfter, collector, isArray) {
        if (objects_1.isDefined(schema.const) && !isArray) {
            collector.add({
    ...
    getInsertTextForSnippetValue(value, separatorAfter, settings, depth) {
        ...
        return json_1.stringifyObject(value, '', replacer, Object.assign(Object.assign({}, settings), { indentation: this.indentation }), depth) + separatorAfter;
    }
