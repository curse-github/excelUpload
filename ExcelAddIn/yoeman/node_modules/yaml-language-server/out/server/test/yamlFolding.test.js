...
describe('YAML Folding', () => {
    ...
    it('should provide folding ranges for multiple documents', () => {
        ...
        const doc = testHelper_1.setupTextDocument(yaml);
        const ranges = yamlFolding_1.getFoldingRanges(doc, context);
        chai_1.expect(ranges.length).to.equal(2);
        chai_1.expect(ranges[0]).to.be.eql(vscode_languageserver_1.FoldingRange.create(1, 2, 0, 7));
        chai_1.expect(ranges[1]).to.be.eql(vscode_languageserver_1.FoldingRange.create(4, 4, 0, 9));
    });
    it('should not include comments on folding ranges', () => {
        ...
        const ranges = yamlFolding_1.getFoldingRanges(doc, context);
        chai_1.expect(ranges.length).to.equal(1);
        chai_1.expect(ranges[0]).to.be.eql(vscode_languageserver_1.FoldingRange.create(3, 4, 0, 11));
    });
    ...
    `;
        const doc = testHelper_1.setupTextDocument(yaml);
        const ranges = yamlFolding_1.getFoldingRanges(doc, context);
        chai_1.expect(ranges).to.deep.include.members([vscode_languageserver_1.FoldingRange.create(1, 2, 4, 14), vscode_languageserver_1.FoldingRange.create(3, 5, 4, 13)]);
    });
...
SecondDict:
  SecondDictFirstKey: foo`;
        const doc = testHelper_1.setupTextDocument(yaml);
        const ranges = yamlFolding_1.getFoldingRanges(doc, context);
        chai_1.expect(ranges).to.deep.include.members([vscode_languageserver_1.FoldingRange.create(1, 3, 2, 11)]);
    });
  ...
  second12:
    name: two`;
        const doc = testHelper_1.setupTextDocument(yaml);
        const ranges = yamlFolding_1.getFoldingRanges(doc, context);
        chai_1.expect(ranges).to.deep.include.members([vscode_languageserver_1.FoldingRange.create(1, 4, 2, 15)]);
    });
    ...
    `;
        const warnings = [];
        const doc = testHelper_1.setupTextDocument(yaml);
        const unlimitedRanges = yamlFolding_1.getFoldingRanges(doc, {
            rangeLimit: 10,
            onRangeLimitExceeded: (uri) => warnings.push(uri),
        });
        chai_1.expect(unlimitedRanges.length).to.equal(2);
        chai_1.expect(warnings).to.be.empty;
        const limitedRanges = yamlFolding_1.getFoldingRanges(doc, {
            rangeLimit: 1,
            onRangeLimitExceeded: (uri) => warnings.push(uri),
        });
        chai_1.expect(limitedRanges.length).to.equal(1);
        chai_1.expect(warnings).to.deep.equal([testHelper_1.TEST_URI]);
    });
});
