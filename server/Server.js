...
tmp.pop();
var dirname = tmp.join("/"); //one back from current path
//#endregion imports/libs
console.clear();
//#region Microsoft Graph
var clientId = fs.readFileSync(dirname + "/graphTokens/ClientId.txt").toString();
var clientSecretValue = fs.readFileSync(dirname + "/graphTokens/ClientSecret.txt").toString();
...
Object.keys(addInPages).forEach(function (i) {
    addInApp.get(i, function (req, res) {
        addInPages[i](req, res, (function (page) { res.sendFile(dirname + "/ExcelAddIn/yoeman/dist" + (page != null ? page : i), "utf8"); }));
    });
});
var transferHandler_1 = require("../transferHandler");
var filename = JSON.parse(fs.readFileSync(dirname + "/config.json").toString()).filename;
var transfers = {};
addInApp.post("/wholeTransfer", function (req, res) {
    ...
    (0, transferHandler_1.handle)(body.data, dirname + "/" + filename + ".json", time);
});
...
addInApp.post("/endTransfer", function (req, res) {
    ...
    if (id != null && transfers[id] != null) {
        ...
        delete transfers[id];
        if (status_1)
            (0, transferHandler_1.handle)(transfer, dirname + "/" + filename + ".json", time);
